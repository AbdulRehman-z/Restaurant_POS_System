...
const Metrics = () => {
  ...
  const { data: statsData, isLoading } = useQuery({
    queryKey: ['dashboardStats', period, customRange],
    queryFn: () => {
    ...
    enabled: !useCustomRange || (customRange.start && customRange.end)
  });

  const kpi = statsData?.data?.data?.kpi || { revenue: 0, orders: 0, aov: 0, netProfit: 0 };
  const trend = statsData?.data?.data?.trend || [];
  ...
  return (
    <div className="space-y-6">
      {/* Header & Filter */}
      <div className="bg-[#1c1c1c] p-6 rounded-2xl border border-white/10 space-y-4">
        <div>
        ...
        <div className="flex flex-wrap gap-4 items-end">
          ...
          <div className="flex bg-[#0a0a0a] p-1 rounded-xl border border-white/10">
            {['day', 'week', 'month', 'year'].map(p => (
              <button
                ...
                className={`px-5 py-2 rounded-lg text-xs font-bold uppercase tracking-widest transition-all ${!useCustomRange && period === p ? 'bg-white text-black' : 'text-gray-500 hover:text-white'}`}
              >
                {p}
              </button>
            ))}
          ...
          <div className="flex items-center gap-2 bg-[#0a0a0a] p-2 rounded-xl border border-white/10">
            <input
              ...
              className="bg-[#1c1c1c] border border-white/10 rounded-lg px-3 py-2 text-xs text-white outline-none focus:border-orange-500"
            />
            ...
            <button
              ...
              className="bg-orange-600 hover:bg-orange-500 disabled:bg-gray-700 disabled:cursor-not-allowed text-white px-4 py-2 rounded-lg text-xs font-bold uppercase tracking-wider transition-all"
            >
              Apply
            </button>
          </div>
